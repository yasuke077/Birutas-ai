...
const ms = require('ms');
const moment = require('moment-timezone'); // Mudado para timezone para precisão no BR

// --- REGISTRO DE FONTES (CORRIGIDO PARA RAILWAY/LINUX) ---
try {
    // Tentamos os dois caminhos mais comuns no Linux Nixpacks
    ...
    console.log("✅ Fontes DejaVu carregadas do sistema.");
} catch (e) {
    console.log("⚠️ Fontes do sistema não encontradas no caminho padrão, usando fallback.");
}
...
// ═══════════════════════════════════════════════════════════════
// 🗄️ BANCO DE DADOS (MONGODB COM PROTEÇÃO)
// ═══════════════════════════════════════════════════════════════
if (!process.env.MONGODB_URI) {
    console.error("❌ ERRO CRÍTICO: A variável MONGODB_URI não foi configurada no Railway!");
    process.exit(1);
}
...
mongoose.connect(process.env.MONGODB_URI, { 
    ...
    console.log('✅ MongoDB: Sincronizado com a Matrix.');
}).catch(err => {
...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        // --- CÓDIGO CORRIGIDO (BADGE SILÊNCIO) ---
        ...
        // --- SISTEMA DE IA COM TROCA DE CARGO, BOTÕES E MENSAGEM DE "PENSANDO" ---
        ...
        if ((isAIChannel || isMentioned) && !isBanned) {
            ...
            // 1. FEEDBACK VISUAL: ATUALIZA O CARGO, NICKNAME E COR DO BOT
            await updateAIRole(message.guild, message.guild.members.me, ia.name, ia.color, allAIs);

            // 2. MANDA A MENSAGEM DE "PENSANDO"
            const thinkingMsg = await message.reply({ content: `🤔 **${ia.name}** está gerando resposta...` }).catch(() => null);
            ...
            const callOpenRouter = async (modelKey) => {
                const currentIA = allAIs[modelKey] || DEFAULT_IAS.gemini;
                try {
                    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                        ...
                        body: JSON.stringify({ 
                            model: currentIA.id, 
                            messages: [{ role: 'system', content: currentIA.prompt }, ...memory.messages], 
                            temperature: 0.7
                        })
                    });
            ...
            if (aiData && aiData.choices && aiData.choices[0]) {
                ...
                // 6. EDITA MENSAGEM COM RESPOSTA
                if (thinkingMsg) {
                    // Divide mensagens longas (limite Discord 2000 chars)
                    if (aiReply.length > 2000) {
                        await thinkingMsg.edit({ content: aiReply.slice(0, 2000), components: [row] }).catch(() => {});
                        const chunks = aiReply.slice(2000).match(/[\s\S]{1,2000}/g);
                        for (const chunk of chunks) {
                            await message.channel.send(chunk).catch(() => {});
                        }
                    } else {
                        await thinkingMsg.edit({ content: aiReply, components: [row] }).catch(() => {});
                    }
                } else {
                    await message.channel.send({ content: aiReply, components: [row] }).catch(() => {});
                }

            } else {
                const finalErrorMsg = "⚠️ Os provedores de IA estão offline. Tente novamente ou use os botões para trocar de modelo.";
                if (thinkingMsg) {
                    await thinkingMsg.edit(finalErrorMsg).catch(() => {});
                } else {
                    await message.reply(finalErrorMsg).catch(() => {});
                }
            }
        }
        await user.save();
...
// ============================
        ...
        if (commandName === 'profile') {
            ...
            ctx.fillText(`💰 Coins: ${td.coins.toLocaleString()}`, 60, 320);
            ctx.fillText(`⭐ Reputação: ${td.reputation}`, 60, 360);
            ctx.fillText(`💍 Casado: ${td.marriedTo ? 'Sim' : 'Não'}`, 60, 400);
            
            // 7. BIOGRAFIA (Com tratamento de quebra)
            ctx.font = 'italic 20px "DejaVu Sans", sans-serif';
            ...
            validBadges.slice(0, 10).forEach((badgeId) => {
                ...
                ctx.font = '42px "DejaVu Sans", "Noto Color Emoji", sans-serif'; 
