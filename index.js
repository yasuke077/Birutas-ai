...
client.on('interactionCreate', async (interaction) => {
    try {
        ...
        if (interaction.isButton() && interaction.customId.startsWith('swap_')) {
            ...
            const allAIs         = { ...DEFAULT_IAS, ...(config.customIAs || {}) };
            const ia             = allAIs[key];
            if (!ia) return interaction.followUp({ content: '❌ IA não encontrada.', ephemeral: true });
            ...
            return interaction.followUp({ content: `✅ IA trocada para **${ia.name}** neste canal.`, ephemeral: true });
        }
        ...
        try {
            const r = { content: '❌ Ocorreu um erro. Tente novamente.', ephemeral: true };
            if (!interaction.replied && !interaction.deferred) await interaction.reply(r);
        } catch { /* silencioso */ }
    }
});
