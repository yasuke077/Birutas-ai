    ...
    try {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('swap_')) {
                ...
                for (const k of Object.keys(allAIs)) {
                    const currentIA = allAIs[k];
                    newRow.addComponents(
                        new ButtonBuilder()
                            .setCustomId(`swap_${k}`)
                            .setLabel(`${currentIA.name} ${currentIA.desc || ''}`)
                            .setStyle(k === key ? ButtonStyle.Success : ButtonStyle.Secondary)
                            .setDisabled(k === key)
                    );
                }
            ...
            // 2️⃣ Botões de casamento (tratados no collector)
        ...
        // 📦 DADOS
        // =======================================================
        const { config, user } = await getData(
        ...
        // =======================================================
        // ⬇️ A PARTIR DAQUI CONTINUA SEUS COMANDOS
        // (hub, profile, adminpanel, etc.)
        // =======================================================
        // EXEMPLO:
        // if (commandName === 'hub') { ... }
...
if (commandName === 'profile') {
    ...
    try {
        ...
        if (!td) {
            return interaction.editReply("❌ Erro ao acessar os dados do usuário.");
        }
        ...
        try {
            ...
            ctx.strokeStyle = td.profileColor || '#ffffff';
            ctx.lineWidth = 6;
            ctx.beginPath();
            ctx.arc(150, 150, 95, 0, Math.PI * 2);
            ctx.stroke();
        } catch (e) {
            console.error('[PROFILE] Erro ao carregar avatar:', e.message);
        }
        ...
        ctx.fillStyle = td.profileColor || '#00ffff';
        ctx.fillRect(280, 165, 430 * progress, 25);

        ctx.fillStyle = '#ffffff';
        ctx.font = '16px "DejaVu Sans"';
        ctx.fillText(`${curXP} / ${nextXP} XP`, 430, 183);
        ...
        ctx.fillText(`Coins: ${td.coins}`, 65, 320);
