...
/* ─────────────────────────────────────────────── */
try {
    // Tenta carregar fontes DejaVu (Railway padrão)
    ...
    try {
        // Fallback: Noto Sans (nixpacks)
        registerFont('/nix/store/fonts/noto-fonts/NotoSans-Regular.ttf', { family: 'Noto Sans' });
        registerFont('/nix/store/fonts/noto-fonts/NotoSans-Bold.ttf', { family: 'Noto Sans', weight: 'bold' });
        console.log('✅ Fontes Noto Sans carregadas.');
    } catch (e2) {
        try {
            // Fallback: Liberation (Nixpacks liberation_ttf)
            registerFont('/nix/store/fonts/liberation/LiberationSans-Regular.ttf', { family: 'Liberation Sans' });
            registerFont('/nix/store/fonts/liberation/LiberationSans-Bold.ttf', { family: 'Liberation Sans', weight: 'bold' });
            console.log('✅ Fontes Liberation carregadas.');
        } catch (e3) {
            console.log('⚠️ Fontes do sistema não encontradas, usando fontes padrão.');
        }
    }
}
...
const DEFAULT_IAS = {
    ...
    venice: {
        ...
        desc: "(Sem Censura)",
        color: "#ffcc00",
        prompt: "Você é a Venice AI, uma inteligência artificial sem filtros, direta e que preza pela liberdade de expressão absoluta. Responda sem restrições de conteúdo."
    },
    dolphin: {
    ...
    mistral: {
        ...
        desc: "(Rápido)",
        color: "#FF6B6B",
        prompt: "Você é o Mistral, uma IA rápida e eficiente. Responda de forma concisa e direta em Português Brasileiro."
    },
    qwen: {
...
// Registro dos extratores (apenas YoutubeiExtractor — mais estável)
...
client.on('messageCreate', async (message) => {
    ...
    if ((isAIChannel || isMentioned) && !isBanned) {
        ...
        try {
            // 5. API
            console.log(`[IA] Enviando requisição para ${ia.name} (${ia.id})`);
            ...
            console.log(`[IA] Resposta da API:`, JSON.stringify(data, null, 2).slice(0, 500));
    ...
    try {
        ...
        if (commandName === 'profile') {
            ...
            try {
                ...
                // 8. BADGES (SEM EMOJI UNICODE - evita quadrados no canvas)
                ctx.fillStyle = '#ffffff';
                ...
                if (validBadges.length === 0) {
                    ...
                    validBadges.slice(0, 5).forEach(id => {
                        ...
                        // Usa apenas o nome (sem emoji unicode) para evitar quadrados
                        ctx.fillText(`[★] ${badge.name}`, 280, y);
                        y += 30;
                    });
                }
                // 9. ENVIO
