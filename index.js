...
// --- REGISTRO MANUAL DE FONTE DO SISTEMA (RAILWAY/LINUX) ---
try {
    // Caminho padrão onde o Nixpacks instala a fonte DejaVu no Linux
    ...
    console.log("✅ Fontes do sistema registradas com sucesso.");
} catch (e) {
    console.log("⚠️ Não foi possível registrar fontes do sistema, tentando usar fallback.");
}
...
const DEFAULT_IAS = {
    gemini: { 
        id: "google/gemini-2.0-flash-001", // ID estável (evita o erro de 'No endpoints')
    ...
    dolphin: { 
        id: "cognitivecomputations/dolphin-2.9.4-llama-3-8b-pro:free", // ID corrigido e mais estável
    ...
    deepseek: { 
        id: "deepseek/deepseek-chat", // DeepSeek costuma funcionar melhor com esse ID no OpenRouter
        name: "DeepSeek R1", 
        color: "#0099ff", 
...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        // --- CÓDIGO CORRIGIDO (LINHAS 512-518) ---
        const lastMsgs = await message.channel.messages.fetch({ limit: 2 }).catch(() => null);
        if (lastMsgs && lastMsgs.size === 2) {
            // .at(1) é o jeito certo de pegar a mensagem anterior no v14
            const lastMsg = lastMsgs.at(1); 
            const diff = Date.now() - lastMsg.createdTimestamp;
            
            if (diff > 604800000) { // 7 dias
                if (!user.badges.includes('silencio')) {
                    user.badges.push('silencio');
                    // Aqui chamamos o verificador para avisar que ganhou a badge
        ...
        if ((isAIChannel || isMentioned) && !isBanned) {
                ...
                try {
                    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                        ...
                        headers: { 
                            ...
                            'HTTP-Referer': 'https://github.com/railwayapp/nixpacks', 
                            'X-Title': 'Birutas AI'
                        },
                        body: JSON.stringify({ 
                            model: modelId, 
                    ...
                    if (data.error) {
                        console.error(`[IA Error] ${modelId}:`, data.error.message);
                        return { error: true };
                    }
                    return data;
                } catch (err) {
            ...
            // --- BLOCO CORRIGIDO PARA ESTABILIDADE DA IA ---
            let aiData = await callOpenRouter(ia.id);
            
            // Fallback 1: Se o modelo principal falhar, tenta a Venice
            if (!aiData || aiData.error || !aiData.choices) {
                console.log(`[IA] Fallback 1 (Venice) ativado para ${message.guild.id}`);
                aiData = await callOpenRouter(DEFAULT_IAS.venice.id);

                // Fallback 2: Se a Venice também falhar, tenta o Gemini estável
                if (!aiData || aiData.error || !aiData.choices) {
                    console.log(`[IA] Fallback 2 (Gemini Stable) ativado para ${message.guild.id}`);
                    aiData = await callOpenRouter("google/gemini-2.0-flash-001");
                }
            }
            ...
            if (aiData && aiData.choices && aiData.choices[0]) {
                ...
                if (aiReply.length > 2000) {
                    ...
                    await message.reply(aiReply).catch(() => {});
                }
            } else {
                // Se tudo falhar, avisa o usuário
                await message.reply("⚠️ Os provedores de IA estão offline no momento. Por favor, tente trocar de IA usando os botões ou aguarde alguns minutos.");
            }
            // -----------------------------------------------
...
// ============================
        ...
        if (commandName === 'profile') {
            ...
            // Criar tela (proporção 16:9 aproximada)
            const canvas = createCanvas(800, 450);
            const ctx = canvas.getContext('2d');
            
            // 1. Fundo Dinâmico (Cor que o usuário escolheu)
            ctx.fillStyle = td.profileColor || '#2c2f33';
            ctx.fillRect(0, 0, 800, 450);
            
            // 2. Overlay de vidro escuro (estilo Loritta/Moderno)
            ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
            ctx.roundRect ? ctx.roundRect(20, 20, 760, 410, 20) : ctx.fillRect(20, 20, 760, 410);
            ctx.fill();
            
            // 3. Avatar Circular com Borda
            try {
            ...
            // 4. TEXTOS (Usando a fonte registrada "DejaVu Sans")
            ctx.fillStyle = '#ffffff';
            
            // Nome do Usuário
            ctx.font = 'bold 45px "DejaVu Sans"'; 
            ctx.fillText(target.username.slice(0, 15), 280, 90);
            
            // Nível
            ctx.font = '30px "DejaVu Sans"';
            ctx.fillText(`Nível ${td.level}`, 280, 140);
            
            // 5. Barra de XP (Visual)
            const currentXP = td.xp;
            const neededXP = Math.floor(100 * Math.pow(td.level + 1, 1.5));
            const percent = Math.min(currentXP / neededXP, 1);
            
            ctx.fillStyle = '#444444';
            ctx.fillRect(280, 160, 450, 25); // Fundo da barra
            ctx.fillStyle = td.profileColor;
            ctx.fillRect(280, 160, 450 * percent, 25); // Progresso
            
            // 6. Estatísticas (Coins e Rep)
            ctx.font = '22px "DejaVu Sans"';
            ctx.fillStyle = '#ffffff';
            ctx.fillText(`💰 Coins: ${td.coins.toLocaleString()}`, 50, 310);
            ctx.fillText(`⭐ Reputação: ${td.reputation}`, 50, 350);
            
            // 7. Biografia
            ctx.font = 'italic 20px "DejaVu Sans"';
            ctx.fillStyle = '#aaaaaa';
            const bioText = td.bio.length > 55 ? td.bio.slice(0, 52) + '...' : td.bio;
            ctx.fillText(`"${bioText}"`, 50, 400);
            
            // 8. BLOCO DE BADGES (CONQUISTAS)
            ctx.fillStyle = '#ffffff';
            ctx.font = 'bold 20px "DejaVu Sans"';
            ctx.fillText("CONQUISTAS:", 280, 230);
            
            let badgeX = 280;
            let badgeY = 270;
            
            // Filtra e desenha apenas as badges que existem no ALL_BADGES
            td.badges.slice(0, 10).forEach((badgeId) => {
                const bInfo = ALL_BADGES[badgeId];
                if (bInfo) {
                    // Emojis usam DejaVu Sans ou o Fallback do sistema
                    ctx.font = '35px "DejaVu Sans", "Noto Color Emoji"'; 
                    ctx.fillText(bInfo.emoji, badgeX, badgeY);
                    badgeX += 48; // Espaço para a próxima
                }
            });
            
            if (td.badges.length === 0) {
                ctx.font = 'italic 18px "DejaVu Sans"';
                ctx.fillStyle = '#555555';
                ctx.fillText("Nenhuma badge conquistada ainda.", 280, 270);
            }
