...
// ============================
        ...
        if (commandName === 'profile') {
            await interaction.deferReply(); // Importante para evitar timeout
            ...
            // Textos - Nome do Usuário
            ctx.fillStyle = '#ffffff';
            ctx.font = 'bold 50px sans-serif'; 
            ctx.fillText(target.username.slice(0, 15), 280, 100);
            
            // Nível e Barra de XP
            ctx.font = '30px sans-serif';
            ctx.fillText(`Nível ${td.level}`, 280, 150);
            
            const currentXP = td.xp;
            const neededXP = xpForLevel(td.level + 1);
            const percent = Math.min(currentXP / neededXP, 1);
            
            ctx.fillStyle = '#444444'; // Fundo da barra
            ctx.fillRect(280, 170, 450, 30);
            ctx.fillStyle = td.profileColor; // Cor da barra
            ctx.fillRect(280, 170, 450 * percent, 30);
            
            // Info de Coins e Rep
            ctx.font = '25px sans-serif';
            ctx.fillStyle = '#ffffff';
            ctx.fillText(`💰 Coins: ${td.coins.toLocaleString()}`, 50, 300);
            ctx.fillText(`⭐ Reputação: ${td.reputation}`, 50, 340);
            
            // Bio (limitada para não vazar da imagem)
            const bio = td.bio.length > 60 ? td.bio.slice(0, 57) + '...' : td.bio;
            ctx.font = 'italic 22px sans-serif';
            ctx.fillText(`"${bio}"`, 50, 400);
            
            // --- BLOCO DAS BADGES (O que faltava no seu!) ---
            let bx = 280; // Posição inicial X das badges
            let by = 250; // Posição inicial Y das badges
            
            td.badges.slice(0, 10).forEach((badgeId) => {
                const bInfo = ALL_BADGES[badgeId];
                if (bInfo) {
                    ctx.font = '35px serif'; // Serif ajuda na renderização de glifos especiais no Linux
                    ctx.fillText(bInfo.emoji, bx, by);
                    bx += 45; // Pula para o lado para a próxima badge
                }
            });
            
            // Se não tiver badges, avisa
            if(td.badges.length === 0) {
                ctx.font = 'italic 18px sans-serif';
                ctx.fillStyle = '#666666';
                ctx.fillText("Sem conquistas ainda...", 280, 250);
            }
