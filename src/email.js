...
function extractErrMsg(err) {
    // Node 18 fetch throws TypeError with .cause = AggregateError
    if (err && err.cause instanceof AggregateError) {
        return err.cause.errors.map(e => e.message || String(e)).join(' | ');
    }
    if (err instanceof AggregateError) {
        return err.errors.map(e => e.message || String(e)).join(' | ');
    }
    if (err && err.cause) return `${err.message}: ${err.cause.message || err.cause}`;
...
async function apiGet(path, token) {
    ...
    try {
        ...
        throw new Error(`Sem conexao com mail.tm: ${extractErrMsg(err)}`);
    }
    if (!r.ok) throw new Error(`mail.tm erro ${r.status}: ${r.statusText}`);
    return r.json();
}
...
async function ensureToken(record) {
    try {
        ...
        // API retornou sem token - conta pode ter expirado
        console.log('[email] ensureToken: sem token na resposta:', JSON.stringify(t).slice(0, 100));
    } catch (e) {
        console.log('[email] ensureToken falhou:', extractErrMsg(e));
    }
        ...
        await interaction.deferReply({ flags: 64 });
            ...
            const rawBody = full.text
                || (full.html || '').replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '')
                               ...
                               .replace(/\s+/g, ' ')
                               .trim();
            // Discord limita campo de embed a 1024 chars; ```\n...\n``` usa 8, sobram 1016
            const body = (rawBody || '(mensagem sem conteudo de texto)').slice(0, 1016);
            ...
            // Log diagnostico completo para identificar o erro real
            console.error('[email_read] type:', err && err.constructor ? err.constructor.name : typeof err);
            console.error('[email_read] message:', err && err.message);
            console.error('[email_read] cause:', err && err.cause);
            if (err && err.errors) console.error('[email_read] errors:', err.errors);
            console.error('[email_read] stack:', err && err.stack ? err.stack.split('\n')[1] : '');
            const msg = extractErrMsg(err);
            try {
                if (interaction.deferred || interaction.replied) {
                    await interaction.editReply({ content: `Erro: ${msg.slice(0, 200)}` });
                } else {
                    await interaction.reply({ content: `Erro: ${msg.slice(0, 200)}`, flags: 64 });
                }
            } catch { /* interaction expirou */ }
        }
    }
